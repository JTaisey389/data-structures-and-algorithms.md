...
const screenForNames = (arr) => {
  ...
  // Step One - Return an array
  // Step Two - We filter the array
  // Step Three - Declaring an arrow function for the value
  // Step Four - The function is to match the specifc qualifiers of the regex to the specified value
};
...
const toTitleCase = (arr) => {
  ...
  // Step One - Return an array
  // Step Two - We map over the array in the same way we use a for loop. Rather than using a .length a .map stops without any further inputs
  // Step Three - We call our function "string" and use an arrow function
  // Step Four - The string.charAt(0) is targeting the letter specifically at index 0 and applying the .toUppercase to change the case of postion ones letter
  // Step Five - The .slice method then targets the charecters in the first position so the remaining charecters are not changed to uppercase 
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  const results = []; // An empty array to pass values into
  const lukeskg = parseInt(starWarsData[0].mass); //LukesKG has to be parsed so we pull in the starWarsData at index 0 for the array and use a .mass
  arr.forEach(charecter => { // so now we call our variable arr and use a forEach to an arrow function. The word "charecter" is potato so it could be anything
    if (charecter.mass > lukeskg){ // If the charecters mass is greater than lukeskg we push the results of the charecters name and join that with a - in the middle
      results.push(charecter.name);
    }
  });
  return results.join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  const matchGame = (rowA, colA, rowB, colB, rowC, colC) => {
    return (board [rowA][colA] !== '') && (board[rowA][colA]) === board[rowB][colB] && (board[rowA][colA] === board[rowC][colC]);};
  if (matchGame(0,0,0,1,0,2)){
    return true;
  }
  if (matchGame(1,0,1,1,1,2)){
    return true;
  }
  if (matchGame(2,0,2,1,2,2)){
    return true;
  }
  if (matchGame(0,0,1,0,2,0)){
    return true;
  }
  if (matchGame(0,1,1,1,2,1)){
    return true;
  }
  if (matchGame(0,2,1,2,2,2)){
    return true;
  }
  if (matchGame(0,0,1,1,2,2)){
    return true;
  }
  if (matchGame(0,2,1,1,2,0)){return true;}
  else {return false;}
};
