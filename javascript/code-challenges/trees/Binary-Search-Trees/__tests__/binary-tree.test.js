...
- If there is no root node, kic0k out of the traversal and return "no root" or "false"
-*/
...
const BinarySearchTree = require('../../Binary-Search-Trees/binary-search-tree');
...
describe('------Test for Trees Part Dux------', () => {
  ...
  it('Should add new nodes to the Binary Search Tree', () => {
    ...
    let testValue = new BinarySearchTree(a);
    testValue.root.left(c);
    testValue.root.right(b);

    expect(testValue.createNode()).tobe(7,28,17);
  });
  // it('Should return PreOrder, PostOrder, and Inorder and let the smoke out when thinking too much about this problem', () => {
  //   const tree = new BinarySearchTree;
  //   tree.addNode(7);
